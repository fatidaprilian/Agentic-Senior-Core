...
- Locked the Phase 1 aggregate-cap strategy after Task 1.5 hit +4.77% OpenAI native `with_loaded_rules` delta versus the Phase 0 baseline. Task 1.5 now stops after 8 migrated rules and defers the 7 remaining pre-migration rule files until a token-offset strategy is approved, preserving the +5% aggregate cap instead of relaxing it for cosmetic consistency.
