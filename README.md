...
```bash
# Clone this repo
git clone https://github.com/YOUR-USERNAME/Agentic-Senior-Core.git
...
1. Copy the `.agent-context/` folder into your project root
2. Copy `.cursorrules` (and/or `.windsurfrules`, `AGENTS.md`) into your project root
3. Open your project in your AI IDE
4. Start coding — the agent will automatically load the rules
...
│   │   ├── performance.md          # N+1 death penalty, measure before optimize
│   │   ├── error-handling.md       # Never swallow, always context
│   │   ├── testing.md              # Test pyramid, AAA pattern, factories
│   │   ├── git-workflow.md         # Conventional Commits, atomic PRs
│   │   └── efficiency-vs-hype.md   # Stable > trendy, justify every dependency
│   │
│   ├── stacks/                     # Language-specific profiles
│   │   └── typescript.md           # Strict TS: no any, Zod everywhere, ESM
│   │
...
### ⚔️ "Galak" Rules (Opinionated & Strict)
...
```
1️⃣  Clone & Open
    git clone this repo → copy into your project → open in Cursor/Windsurf

2️⃣  The Initiation Prompt
    "Build [Project]. Scan .agent-context/ and scaffold from blueprints/api-nextjs.md"

3️⃣  Agent Builds Like a Pro
...
Run the Bun-powered validator to check repository integrity:
...
- ✅ 8 universal rule files
- ✅ TypeScript stack profile
- ✅ Next.js & NestJS blueprints
- ✅ PR & security checklists
...
1. Fork the repo
2. Add your rule/stack/blueprint
3. Run `bun scripts/validate.ts` to verify
4. Open a PR with a clear description

### Content Quality Standard
Every file must pass the **"galak" test**:
- ❌ `"Use good naming"` → generic, useless
- ✅ `"NEVER use single-letter variables except i in for-loops. Boolean MUST use is/has/can prefix."` → specific, enforceable
...
⭐ Star this repo if you believe AI agents should have standards.
