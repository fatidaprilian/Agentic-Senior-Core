...
async function installAntigravityIde(target) {
  ...
  // Copy hooks.json to global root hook locations so Antigravity CLI and IDE detect global hooks natively
  const hooksSource = path.join(REPOSITORY_ROOT, '.agents', 'plugins', 'agentic-senior-core', 'hooks.json');
  if (await pathExists(hooksSource)) {
    const rootHookTargets = [
      path.join(HOME, '.gemini', 'antigravity-cli', 'hooks.json'),
      path.join(HOME, '.gemini', 'config', 'hooks.json'),
      path.join(HOME, '.gemini', 'hooks.json'),
    ];
    for (const hookTarget of rootHookTargets) {
      try {
        await fs.mkdir(path.dirname(hookTarget), { recursive: true });
        await fs.copyFile(hooksSource, hookTarget);
      } catch (_) {}
    }
  }
