...
export async function registerCliSmokeAuditsAndOpsTests(t) {
  ...
  await t.test('frontend usability audit outputs machine-readable report', () => {
    ...
    assert.match(frontendRuleContent, /Load this rule for UI-facing work\. Keep the loaded surface small\./);
    assert.match(frontendRuleContent, /Treat `\.agent-context\/` as design governance authority\./);
    assert.match(frontendRuleContent, /Do not choose final style, framework, palette, typography, layout paradigm, or animation library offline\./);
