...
export async function registerDesignContractSeedSmokeTests(t) {
  await t.test('init scaffolds a dynamic UI design contract for web projects', () => {
    ...
    try {
      ...
      assert.match(bootstrapDesignPrompt, /Section 3 — Category Code Identification/);
      assert.match(bootstrapDesignPrompt, /Section 4 — Morphological Exploration/);
      assert.match(bootstrapDesignPrompt, /Section 5 — Anchor Candidates/);
      ...
      assert.equal(designIntentSeed.conceptualAnchor.researchBrief, '.agent-context/prompts/research-design.md');
      assert.equal(designIntentSeed.conceptualAnchor.categoryCodes.mode, 'agent-must-complete-before-ui-implementation');
      assert.equal(designIntentSeed.conceptualAnchor.categoryCodes.blockingByDefault, true);
      assert.equal(designIntentSeed.conceptualAnchor.categoryCodes.researchBriefSection, 'Section 3');
      assert.equal(designIntentSeed.conceptualAnchor.categoryCodes.minimumEntries, 3);
      assert.match(designIntentSeed.conceptualAnchor.categoryCodes.specificityRule, /visualize a specific aesthetic direction/);
      assert.match(designIntentSeed.conceptualAnchor.categoryCodes.antiLeakageRule, /trap/);
      assert.match(designIntentSeed.conceptualAnchor.categoryCodes.selfTestRule, /Read each category code aloud/);
      assert.equal(designIntentSeed.conceptualAnchor.categoryCodes.failingExamples.length >= 3, true);
      assert.equal(designIntentSeed.conceptualAnchor.categoryCodes.passingExamples.length >= 3, true);
      assert.match(designIntentSeed.conceptualAnchor.categoryCodes.passingExamplesPolicy, /illustrate the description format/);
      assert.equal(designIntentSeed.conceptualAnchor.categoryCodes.commonAiSafeClichesToReject.length >= 3, true);
      assert.match(designIntentSeed.conceptualAnchor.categoryCodes.commonAiSafeClichesPolicy, /rejection note/);
      assert.ok(designIntentSeed.conceptualAnchor.categoryCodes.requiredFieldsPerEntry.includes('specificityEvidence'));
      assert.ok(designIntentSeed.conceptualAnchor.categoryCodes.requiredFieldsPerEntry.includes('rejectionNote'));
      assert.ok(designIntentSeed.conceptualAnchor.categoryCodes.forbiddenPlaceholderPhrases.includes('clean typography'));
      assert.deepEqual(designIntentSeed.conceptualAnchor.categoryCodes.candidateEntries, []);
      assert.equal(designIntentSeed.conceptualAnchor.morphologicalExploration.researchBriefSection, 'Section 4');
      assert.equal(designIntentSeed.conceptualAnchor.morphologicalExploration.requiredMatrixShape, '5x5-or-6x5');
      assert.equal(designIntentSeed.conceptualAnchor.morphologicalExploration.minimumDimensions, 5);
      assert.equal(designIntentSeed.conceptualAnchor.morphologicalExploration.maximumDimensions, 6);
      assert.equal(designIntentSeed.conceptualAnchor.morphologicalExploration.valuesPerDimension, 5);
      assert.equal(designIntentSeed.conceptualAnchor.morphologicalExploration.forbidCategoryCodeValuesInMatrix, true);
      assert.equal(designIntentSeed.conceptualAnchor.morphologicalExploration.requireUncomfortableCombination, true);
      assert.match(designIntentSeed.conceptualAnchor.morphologicalExploration.uncomfortableCombinationRule, /uncomfortable combination/);
      assert.ok(designIntentSeed.conceptualAnchor.morphologicalExploration.uncomfortableCombinationRequiredFields.includes('combinationLabel'));
      assert.ok(designIntentSeed.conceptualAnchor.morphologicalExploration.uncomfortableCombinationRequiredFields.includes('discomfortReason'));
      assert.ok(designIntentSeed.conceptualAnchor.morphologicalExploration.uncomfortableCombinationRequiredFields.includes('productLogicJustification'));
      assert.equal(designIntentSeed.conceptualAnchor.morphologicalExploration.widenDimensionsWhenUncomfortableCombinationCannotBeProduced, true);
      assert.equal(designIntentSeed.conceptualAnchor.morphologicalExploration.uncomfortableCombination, null);
      assert.equal(designIntentSeed.conceptualAnchor.anchorCandidates.researchBriefSection, 'Section 5');
      assert.equal(designIntentSeed.conceptualAnchor.anchorCandidates.requiredCandidateCount, 5);
      assert.ok(designIntentSeed.conceptualAnchor.anchorCandidates.requiredFieldsPerCandidate.includes('renameTest'));
      assert.ok(designIntentSeed.conceptualAnchor.anchorCandidates.requiredFieldsPerCandidate.includes('categoryCodeOverlap'));
      assert.equal(designIntentSeed.conceptualAnchor.anchorCandidates.renameTest.mode, 'strengthened');
      assert.equal(designIntentSeed.conceptualAnchor.anchorCandidates.renameTest.requiredTestCategoryCount, 3);
      assert.match(designIntentSeed.conceptualAnchor.anchorCandidates.renameTest.testCategoryDistanceRule, /remote from each other/);
      assert.match(designIntentSeed.conceptualAnchor.anchorCandidates.renameTest.testCategoryFreshnessRule, /Pick fresh test categories/);
      assert.equal(typeof designIntentSeed.conceptualAnchor.anchorCandidates.renameTest.verdictScoring['STRONG PASS'], 'string');
      assert.equal(typeof designIntentSeed.conceptualAnchor.anchorCandidates.renameTest.verdictScoring.PASS, 'string');
      assert.equal(typeof designIntentSeed.conceptualAnchor.anchorCandidates.renameTest.verdictScoring.REVISE, 'string');
      assert.equal(typeof designIntentSeed.conceptualAnchor.anchorCandidates.renameTest.verdictScoring.DISCARD, 'string');
      assert.ok(designIntentSeed.conceptualAnchor.anchorCandidates.renameTest.renameTestRequiredFields.includes('testCategories'));
      assert.ok(designIntentSeed.conceptualAnchor.anchorCandidates.renameTest.renameTestRequiredFields.includes('verdict'));
      assert.ok(designIntentSeed.conceptualAnchor.anchorCandidates.forbiddenSelectedVerdicts.includes('DISCARD'));
      assert.ok(designIntentSeed.conceptualAnchor.anchorCandidates.requiredSelectedVerdicts.includes('STRONG PASS'));
      assert.ok(designIntentSeed.conceptualAnchor.anchorCandidates.requiredSelectedVerdicts.includes('PASS'));
      assert.deepEqual(designIntentSeed.conceptualAnchor.anchorCandidates.candidates, []);
